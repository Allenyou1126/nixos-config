...
let
  ...
  mkRecord =
    record: "${record.name} ${toString record.ttl} ${record.class} ${record.type} ${record.value}";
  mkSoaRecord =
    soa:
    "@ IN SOA ${soa.mname} ${soa.rname} ${toString soa.serial} ${toString soa.refresh} ${toString soa.retry} ${toString soa.expire} ${toString soa.minimum}\n";
  mkZoneFile =
    zone:
    let
  ...
  mkCoreDnsConfig =
    ...
    let
      renderServerBlock =
        ...
        + (
          if lib.length server.plugins > 0 then
            (lib.concatStringsSep "\n" (
              map (
                plugin: ("    " + (if lib.isList plugin then lib.concatStringsSep " " plugin else plugin))
              ) server.plugins
            ))
            + "\n"
          else
