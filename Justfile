...
switch host="":
  sudo nixos-rebuild switch --flake .{{ if host == "" { "" } else  { "#" + host } }} --use-remote-sudo

# Build and switch to a new generation using cn mirrors. Usage: just switch-cn hostname(leave empty for default)
switch-cn host="":
  sudo nixos-rebuild switch --flake .{{ if host == "" { "" } else  { "#" + host } }} --use-remote-sudo --override-input nixpkgs "git+https://mirrors.nju.edu.cn/git/nixpkgs.git?ref=nixos-25.11&shallow=1" --override-input home-manager "git+https://gitee.com/Allenyou1126/home-manager?ref=release-25.11&shallow=1"
